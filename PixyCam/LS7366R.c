 ...
 * Author: Andrew Friesen
 * Date: Feb 1, 2022
 * Modified: 2022/02/11 by Jamie Boyd
 * reversed order of filling the array so that a single cast effectively replaces bit-shifting on small endian msp430
 ************************************************************************************/
unsigned char LS7366Rread(unsigned char reg, unsigned char * dataIn)
{
    ...
    else if(reg==CNTR||reg==OTR)//If you're reading an n-byte register
    {
 ...
 * Date: Feb 1, 2022
 ************************************************************************************/
unsigned char LS7366Rwrite(unsigned char reg, unsigned char *dataOutP)
{
    int i=0;
