...
volatile unsigned char spiTxPos =0;
...
// returns length of txBuffer, or 0 if "dirty" non-numeric characters were found
...
void usciB1SpiTXBuffer (const unsigned char * buffer, int bufLen){
    unsigned char iBuf;
#ifdef SECTION_523
        while (!(UCB1IFG & UCTXIFG)){};   // poll TXIFG, waiting for an opportunity to send
        P6OUT &= ~1;        // assert /SS sending it low (active low signal)
#endif
    for (iBuf =0; iBuf < bufLen; iBuf +=1){
        usciB1SpiPutChar(buffer[iBuf]);
    }
#ifdef SECTION_523
        while (!(UCB1IFG & UCRXIFG)){};   // poll, waiting while last byte is sent
        UCB1IFG &= ~UCRXIFG;            // clear flag for next time
        P6OUT |= 1;                     // un-assert /SS
#endif
}
